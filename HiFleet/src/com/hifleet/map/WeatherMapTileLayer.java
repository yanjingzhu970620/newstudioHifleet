...
public class WeatherMapTileLayer extends BaseMapLayer {
	...
	public WeatherMapTileLayer(boolean mainMap) {
	...
	public void onPrepareBufferImage(Canvas canvas, RotatedTileBox tileBox,
		...
		if (app.myPreferences.getBoolean("IsShowAirPressure", false)
				&& tileBox.getZoom() <= 7) {
			...
			// if(tileBox.getLongitude()<=0 || tileBox.getLatitude()<=0){
			// //System.err.println("tileBox.getLongitude <=0 return...");
			// return;
			// }
			...
			// for(DMapRefreshCallback call :new
			// ArrayList<DMapRefreshCallback>(calls)){
	...
	public void drawTileMap(Canvas canvas, RotatedTileBox tileBox) {
		// System.err.println("  green   MapTileLayer draw tile map ?"+this.map.getName());
		...
		int maxLevel = 16;// Math.min(view.getSettings().MAX_LEVEL_TO_DOWNLOAD_TILE.get(),
				...
				if (bmp == null) {
					...
					if (bmp != null) {
						...
						if (!useSampling) {
							...
							Bitmap s = Bitmap.createBitmap(scaledSize + 2
									* margin - 1, scaledSize + 2 * margin - 1,
									Config.ARGB_8888);
							Bitmap sampled = s.createBitmap(bmp,
					...
					bitmapToZoom.set(0, 0, tileSize, tileSize);
					canvas.drawBitmap(eraseColor(bmp), bitmapToZoom,
							bitmapToDraw, paintBitmap);
				}
...
//		System.err.println("刷weather图结束*************发出通知 ");
		// for(DMapRefreshCallback call :new
		// ArrayList<DMapRefreshCallback>(calls)){
		// call.dotmapRefreshed(tileBox);
		// }
	...
	// private List<DMapRefreshCallback> calls = new
	// ArrayList<DMapRefreshCallback>();
	//
