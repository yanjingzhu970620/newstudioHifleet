...
public class AnimateDraggingMapThread {
	...
	private final static float DRAGGING_ANIMATION_TIME = 600f;//1200f;
	private final static float ZOOM_ANIMATION_TIME = 400f;//800f;
	private final static float ZOOM_MOVE_ANIMATION_TIME = 300f;//650f;
	private final static float MOVE_MOVE_ANIMATION_TIME = 1000f;//2000f;
	private final static int DEFAULT_SLEEP_TO_REDRAW = 20;//55;
...
//		System.err.println("AnimateDraggingMapThread pendingRotateAnimation!!!");
		if (targetRotate != -720) {
			do {
				...
				if (absDiff > 0) {
					...
					if (absDiff < 1) {
						...
						tileView.rotateToAnimateNew(targetRotate);
					} else {
						conditionToCountinue = true;
						//tileView.rotateToAnimate(rotationDiff / 5 + tileView.getRotate());
						
						tileView.rotateToAnimateNew(rotationDiff / 5 + tileView.getRotate());
					}
				}
	...
	public void startMoving(final double finalLat, final double finalLon, final int endZoom, final float endZoomScale, final boolean notifyListener){
		stopAnimatingSync();
		System.err.println("AnimateDraggingMapThread startMovingstartMoving!!!");
		...
		if (skipAnimation) {
			//System.out.println("skipAnimation. set and return.");
		...
		startThreadAnimating(new Runnable() {
			...
			public void run() {
				...
				if(!stopped){
					tileView.setLatLonAnimate(finalLat, finalLon, notifyListener);
					//print("finalLon: "+finalLon+", finalLat: "+finalLat);
				}
	...
	public void startZooming(final int zoomEnd, final float zoomScale, final boolean notifyListener){
		...
		startThreadAnimating(new Runnable(){
			...
			public void run() {
				...
				pendingRotateAnimation();
				//System.out.println("缩放线程run结束了。。。");
				//tileView.callMapMoveEndDraw();
			}
		}); //$NON-NLS-1$
	}
	...
	public void startDragging(final float velocityX, final float velocityY, float startX, float startY, 
		...
		startThreadAnimating(new Runnable(){
			@Override
			public void run() {
				System.err.println("AnimateDraggingMapThread startDragging!!!");
				float curX = endX;
				float curY = endY;
	...
	public void startRotate(final float rotate) {
		if (!isAnimating()) {
			//System.err.println("clear target values..");
			clearTargetValues();
			// stopped = false;
			...
			startThreadAnimating(new Runnable() {
				...
				public void run() {
					targetRotate = rotate;
					//System.err.println("pendingRotateAnimation");
					pendingRotateAnimation();
				}
			});
		} else {
