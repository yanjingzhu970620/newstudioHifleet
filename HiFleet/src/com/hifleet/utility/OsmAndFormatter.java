...
public class OsmAndFormatter {
	...
	public static String getFormattedBearing(double bearing){
		DecimalFormat df = new DecimalFormat("###.0");
		return df.format(bearing);
	}
	
	public static String getFormattedBearingForDistanceCalculatorLayer(double bearing){
	...
	public static String getFormattedDistance(float meters, OsmandApplication ctx) {
		...
		if (meters >= 100 * mainUnitInMeters) {
			...
			//return (int) (meters / mainUnitInMeters + 0.5) + " " + ctx.getString(mainUnitStr); //$NON-NLS-1$//原版
			return (int) ((meters / mainUnitInMeters + 0.5)/1.852)+" "+ctx.getString(mainUnitStrNautical);
			
		} else if (meters > 9.99f * mainUnitInMeters) {
			
			//return MessageFormat.format("{0,number,#.#} " + ctx.getString(mainUnitStr), ((float) meters) / mainUnitInMeters); //$NON-NLS-1$
			return MessageFormat.format("{0,number,#.#} " + ctx.getString(mainUnitStrNautical), ((float) meters) / (mainUnitInMeters*1.825)); //$NON-NLS-1$
		} else if (meters > 0.999f * mainUnitInMeters) {
			
			
			//return MessageFormat.format("{0,number,#.##} " + ctx.getString(mainUnitStr), ((float) meters) / mainUnitInMeters); //$NON-NLS-1$
			
			return MessageFormat.format("{0,number,#.##} " + ctx.getString(mainUnitStrNautical), ((float) meters) / (mainUnitInMeters*1.852)); //$NON-NLS-1$
