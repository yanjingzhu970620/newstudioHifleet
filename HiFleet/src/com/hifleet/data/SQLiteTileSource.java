...
import com.hifleet.map.SQLiteAPI;
import com.hifleet.map.SQLiteAPI.SQLiteConnection;
import com.hifleet.map.SQLiteAPI.SQLiteCursor;
...
public class SQLiteTileSource implements ITileSource {
	...
	public boolean exists(int x, int y, int zoom) {
		...
		try {
			...
			//“select 1 from tile where x=”+x" and y="+y+" and z="+z;
...
//			}
	...
	public void deleteImage(int x, int y, int zoom) {
		...
		db.execSQL("DELETE FROM tiles WHERE x = ? AND y = ? AND z = ?", 
