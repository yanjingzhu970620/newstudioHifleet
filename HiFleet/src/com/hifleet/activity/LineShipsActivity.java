...
public class LineShipsActivity extends HBaseActivity {
	...
	ListView mListShips;
	ListView mListShipsEnd;
	TextView mtitle;
	TextView startportname;
	TextView endportname;
	LinearLayout end_portedit;
	OsmandApplication app;

	
	static PortListAdapter mAdapter;
	static PortListAdapter mAdapterEnd;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mListShipsEnd = (ListView) findViewById(R.id.list_ports);
		...
		mEditPortEnd.addTextChangedListener(portsWatcherEnd);
		...
		mtitle.setText("航线船舶");
		startportname=(TextView) findViewById(R.id.start_port);
		startportname.setText("起始港口");
		endportname=(TextView) findViewById(R.id.end_port);
		endportname.setText("终止港口");
		end_portedit=(LinearLayout) findViewById(R.id.end_porteditl);
		end_portedit.setVisibility(View.VISIBLE);
...
//		mAdapterEnd = new PortListAdapter(searchActivity, searchPortBean,mEditPortEnd);
//		mListShips.setAdapter(mAdapterEnd);
	}
...
//			mAdapter.mPort;
	...
	private TextWatcher portsWatcherEnd = new TextWatcher() {
		...
		public void afterTextChanged(Editable s) {
			// TODO Auto-generated method stub
			String nameString = mEditPortEnd.getText().toString();
//			mAdapter.mPort;
			portName=null;
			portName = nameString.replace(" ", "%20");
			if (allsearchPortBean.size() > 0) {
			...
			if (!nameString.equals("")) {
				mAdapter = new PortListAdapter(searchActivity, searchPortBean,mEditPortEnd);
				mListShips.setAdapter(mAdapter);
				pt="ep";
				LodingportsNameThread searchship = new LodingportsNameThread();
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
...
//					System.out.println("解析的inportships"+is.mmsi);
	...
	class LodingLineShipsThread extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			try {
				String httpPost = app.myPreferences.getString("loginserver",
						null) + IndexConstants.LINE_SHIPS_URL + sdwt+"&limit1="+"0"+"&limit2="+"10"+"&startportcode="+
						startport.getPortCode()+"&endportcode="+endport.getPortCode()+"&type="+stype;
				System.out.println(httpPost);
				URL shipsUrl = new URL(httpPost);
				HttpURLConnection conn = (HttpURLConnection) shipsUrl
						.openConnection();
...
//				System.out.println("点击了："+inportships.size() +"进行跳转");
				Intent intent = new Intent(LineShipsActivity.this, PortShipsActivity.class);
...
//				progress.setVisibility(View.GONE);
				...
				progressLoading.setVisibility(View.GONE);
				Toast.makeText(getBaseContext(), "暂无航线船舶", Toast.LENGTH_LONG).show();
			}
