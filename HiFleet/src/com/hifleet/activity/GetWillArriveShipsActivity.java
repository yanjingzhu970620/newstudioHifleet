...
public class GetWillArriveShipsActivity extends HBaseActivity {
	...
	public static String stime="0";
	public static String etime="0";
...
//				System.out.println("查询港口船舶"+sdwt+stype+portName);
				...
				stime="0";
				etime="0";
				stime=starttime.getText().toString().replace(" ", "%20");
//					starttime.setText(stime);
				etime=endtime.getText().toString().replace(" ", "%20");
//					endtime.setText(etime);
...
//				s=System.currentTimeMillis();
				...
				conn.setConnectTimeout(20000);
				System.out.println("是否能获取网络数据"+conn.getResponseCode());
				InputStream inStream = conn.getInputStream();
//				System.out.println("预计抵达船舶  请求完成  花费时间：：："+(System.currentTimeMillis()-s));
				parseXMLnew(inStream);
//				System.out.println("预计抵达船舶  解析完成  花费时间：：："+(System.currentTimeMillis()-s));
				inStream.close();
			} catch (Exception e) {
				// TODO: handle exception
				System.out.println("未能获取网络数据");
		...
		protected void onPostExecute(Void result) {
			// TODO Auto-generated method stub
			super.onPostExecute(result);
//			System.out.println("预计抵达船舶  线程完成  花费时间：：："+(System.currentTimeMillis()-s));
...
//				progress.setVisibility(View.GONE);
				...
				progressLoading.setVisibility(View.GONE);
				Toast.makeText(getBaseContext(), "暂无预抵船舶", Toast.LENGTH_LONG).show();
			}
