 ...
 *
 */
public class TeamShipsAlertMessageActivity extends Activity  implements IXListViewListener {
...
//		progress = (ProgressBar) findViewById(R.id.progress);
		...
		mAdapter = new AlertMessageAdapter(this,"shipalert","",inportships);
		...
		LodingShipsAlertMsgThread LodingShips=new LodingShipsAlertMsgThread();
...
//	private HashMap<String, LodingLineShipsThread> ltaskmap = new HashMap<String, LodingLineShipsThread>();//获取航线船舶的线程
	private HashMap<String, LodingShipsAlertMsgThread> areataskmap = new HashMap<String, LodingShipsAlertMsgThread>();//获取在港船舶的线程
...
//			mtitle.setText("区域船舶");
			...
			while (it.hasNext()) {
				String uuid = it.next();
				LodingShipsAlertMsgThread task = areataskmap.get(uuid);
				task.cancel(true);
			}
	}
...
//							PortBean.class));
//				}
				if (childElement.getNodeName().compareTo("AlertRss") == 0) {
...
//			handler.sendMessage(message);
	...
	class LodingShipsAlertMsgThread extends AsyncTask<String, Void, Void> {
		/*
		 * (non-Javadoc)
		 * 
		...
		protected Void doInBackground(String... params) {
			...
			try {
				String httpPost = app.myPreferences.getString("loginserver",
						null) + IndexConstants.GET_SHIPS_ALERT_MESSAGE_URL +mmsi+"&limit1="+limit1+"&limit2="+limit2;
				System.out.println("GET_SHIPS_ALERT_MESSAGE_URL::"+httpPost);
